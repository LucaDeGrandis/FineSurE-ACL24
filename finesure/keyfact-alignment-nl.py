...
import sys
import os
from utils import compute_completeness_percentage_score
from utils_nl import get_keyfact_alighment_prompt, get_response, parsing_llm_keyfact_alighment_output, compute_conciseness_percentage_score


async def main(input_path, keyfact_path, output_path, print_interval=2):
    '''
    Argument:
    ...
    for input_id, input_json in enumerate(inputs):
        ...
        messages_list.append(get_keyfact_alighment_prompt(summary_sentences=list_keyfacts, source_document=summary))
    tasks = [get_response(client=_client, messages=messages, model=_model) for messages in messages_list]
    ...
    for input_id, (input_json, output) in enumerate(zip(inputs, responses)):
        ...
        input_json['pred_alignment_labels'], input_json['pred_sentence_line_numbers'] = parsing_llm_keyfact_alighment_output(output, summary)
        ...
        print('\t[Matched Sentence Lines]:', len(input_json['pred_sentence_line_numbers']))
...
if __name__ == "__main__":
    ...
    asyncio.run(main(input_path, keyfact_path, output_folder, print_interval))
