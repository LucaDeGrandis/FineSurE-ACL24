...
async def main(input_path, output_path, print_interval=2):
    ...
    for input_id, input_json in enumerate(inputs):
        ...
        messages_list.append(get_fact_checking_prompt(summary_sentences=sentences, document=src))
    tasks = [get_response(client=_client, prompt=prompt, model=_model) for prompt in messages_list]
